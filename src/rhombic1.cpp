...
void buildSliceOne(vint &lowerQubitIndices, vint &middleQubitIndices, vint &upperQubitIndices, vint &syndIndices, vint &defectIndices, vint &bulkSweepVertices, vint &middleBoundarySweepVertices, vint &upperBoundarySweepVertices1, vint &upperBoundarySweepVertices2, vint &zLogical, vvint &vertexToFaces, vvint &faceToVertices, vvint &faceToEdges, vpint &edgeToVertices, int L)
{
    ...
    while (!generatorVertices.empty())
    {
        ...
        if (c.xi[2] == (L-3))
        {
            if (c.xi[1] != (L-3))
            {
                syndIndices.push_back(4*v + xyz);
                syndIndices.push_back(4*(v+5*L) + xz);
                syndIndices.push_back(4*(v+1) + yz);
                syndIndices.push_back(4*neigh(v,xyz,1,L) + xy);
                ...
                if (c.xi[1] != 1) defectIndices.push_back(v);
            }
        }
        if (c.xi[2] == 1)
        {
            if (c.xi[1] != 1)
            {
                syndIndices.push_back(4*neigh(v,yz,-1,L) + xz);
                syndIndices.push_back(4*neigh(v,yz,-1,L) + yz);
                syndIndices.push_back(4*neigh(v,yz,-1,L) + xyz);
                defectIndices.push_back(neigh(v,yz,-1,L));
            }
            if (c.xi[1] != (L-3)) syndIndices.push_back(4*(v+1) + xy);
        }
    ...
    //These are the lower and middle bulk stabilisers
    ...
    {
        if (p.second == 3) syndIndices.push_back(p.first);
    }
    ...
    for (std::pair<int, int> p : vertexCount)
    {
        if (p.second == 4 &&
           (indexToCoord(p.first, L).xi[3] == 1 || 
            indexToCoord(p.first, L).xi[2] == L-2 ||
    ...
    for (int face : lowerQubitIndices)
    {
        ...
        for (int vertex : vertices)
        {
            ...
            else if (((c.xi[0] + c.xi[1] + c.xi[2]) % 2 == 1) && c.xi[1] == 1)
            {
    ...
    for (int face : upperQubitIndices)
    {
        ...
        for (int vertex : vertices)
        {
            ...
            {
                bulkSweepVerticesSet.insert(vertex);
                if (c.xi[1] == 1) upperBoundarySweepVertices2Set.insert(vertex);
                else if (c.xi[1] == (L-3)) upperBoundarySweepVertices2Set.insert(vertex);
            }
            else if (c.xi[3] == 0 && c.xi[2] == (L-2))
            {
